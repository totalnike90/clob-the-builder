...
1. Read `<repo-root>/.taskstate/$ARGUMENTS.json`. Confirm:
   - `status: reviewed` (per [ADR 0037](../../decisions/0037-walk-before-gates-iterative.md), supersedes ADR 0007 which gated on `walked`). `walked` → run `/check <ID>` then `/review <ID>`. `built` → run `/walk <ID>` (UX iteration or non-UX auto-skip), then `/check`, then `/review`. `in-progress` → build not finished. No overrides without an ADR.
...
Lock lives in the **main checkout** (repo root), not the worktree. Path: `<repo-root>/.ship.lock` (gitignored). The lock carries owner PID + acquisition timestamp so a crashed session doesn't wedge future ships. TTL: 30 minutes. Stale (dead PID **or** age >30 min) → remove and retry once.
...
4. **Fast path** — if `MAIN_BEFORE = MAIN_AFTER`: `origin/main` hasn't advanced since `/check` ran; the worktree code is identical to what already passed quality gates. Skip the rebase and the quality-gate re-run entirely.
   ...
   **Slow path** — if `MAIN_BEFORE ≠ MAIN_AFTER`: a sibling shipped; rebase and re-verify:
   ...
   - Re-run cheap quality gates **in parallel** (saves 5–15s over sequential `&&`):
...
- Task ID, subject, PRD refs (trailer keys per `commit.trailer_keys` in `ritual.config.yaml`)
- Branch name (from sidecar)
- Files touched: `git diff --name-only main...HEAD`
- Migrations added (if the project has a `migrations/` path): `git diff --name-only main...HEAD -- <migrations-path>/`
...
## Step 7.5 — triage follow-ups, pre-merge (per [ADR 0006](../../decisions/0006-followup-triage-register.md))
...
Otherwise, list all bullets and offer a batch option first:
...
7. On `main` (main checkout):
   ...
   - Append a new block to `BUILDLOG.md` using the template (newest on top). Include date, Task ID + subject, Branch (now deleted), PR URL or `local`, squash SHA, full file list, migrations added (if applicable), acceptance criteria with check marks, revert command, follow-ups. **Wrap in stable anchors** (per [ADR 0021](../../decisions/0021-reduce-context-bloat-buildlog-followups.md)):

     ```markdown
     <!-- TASK:<ID>:START -->

     ## YYYY-MM-DD HH:MM — <ID> <subject>
...
- If a migration shipped, the revert command in BUILDLOG must include the specific down-migration or equivalent rollback step.
