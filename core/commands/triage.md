...
List all unregistered bullets and offer a batch option first (per [ADR 0020](../../decisions/0020-batch-dispositions-ship-step-7-5.md)):
...
```
Source: F-8
Bullet: "Add exceptions_to_handle=(RateLimitError, ServiceUnavailableError, TimeoutError) to ProviderChain.build()"
...
1. Ask: **"Target phase?"** (F / SC / AU / LI / AG / CH). Default to the source task's phase.
...
6. Ask: **"PRD refs?"** (e.g. `§9.8, §9.9`). Default to source task's PRD refs.
...
3. Optional: ask for a line ref (e.g. `.github/workflows/ci.yml:48`) — included in `note`.
...
- `--all` is backfill only. After first run on ADR-0006 adoption, use single-ID mode in the normal ship flow.
